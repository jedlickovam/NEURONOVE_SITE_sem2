...
for i =1:P
    ...
    I  = imread(nazev,'jpg');         % nacteni obázků a převod do cernobíle  
    P1 = im2bw(I);
    
    %figure(i);
    %imshow(P1);
    
    P1 = mat2vec(P1);                % převod na vektor
    P1 = double(P1);
      
    for n = 1:length(P1)             % potřeba aby nebylo jen 1 a 0 ale 1 a -1 
        if P1(n) == 0;
            P1(n) = -1;
        end
    end
    
    up(:,i) = P1;                    % vytvoření prototypů
    up = double(up);
    
    R1 = im2bw(I);                   % přidání šumu k původnímu obrazu
    ...
    ur(:,i) = R1;                    % hotové zašuměnné obrazy
    ur = double(ur);
end
